# Graph Report - pake-mail  (2026-07-29)

## Corpus Check
- 116 files · ~75,228 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 694 nodes · 1213 edges · 52 communities (42 shown, 10 thin omitted)
...
1. `requireUserId()` - 74 edges
2. `handleApi()` - 73 edges
3. `apiSuccess()` - 70 edges
4. `apiError()` - 58 edges
5. `cn()` - 30 edges
...
- `POST()` --calls--> `encrypt()`  [EXTRACTED]
  app/api/email-accounts/route.ts → lib/encryption.ts
- `BatchDetail()` --calls--> `cn()`  [EXTRACTED]
  components/batches/batch-detail.tsx → lib/utils.ts
- `BatchWizard()` --calls--> `cn()`  [EXTRACTED]
  components/batches/batch-wizard.tsx → lib/utils.ts
...
Cohesion: 0.10
Nodes (58): POST(), GET(), POST(), DELETE(), POST(), POST(), DELETE(), GET() (+50 more)
...
Cohesion: 0.10
Nodes (23): features, footerLinks, LoginForm(), DashboardShell(), SidebarContext, SidebarContextType, useSidebar(), Header() (+15 more)
...
Cohesion: 0.17
Nodes (10): DashboardContent(), DashboardStats, ActivityItem, EVENT_ICONS, RecentActivity(), UpcomingBatchItem, UpcomingSchedule(), Badge() (+2 more)
...
### Community 12 - "cn"
Cohesion: 0.13
Nodes (15): RunningBatch(), RunningBatchItem, Dialog(), DialogProps, Label, LabelProps, fillColorMap, ProgressBar() (+7 more)

### Community 13 - "storage.ts"
Cohesion: 0.11
Nodes (27): GET(), categorizeError(), ERROR_PATTERNS, ErrorCategory, decrypt(), encrypt(), KEY, AUTO_REPLY_PATTERNS (+19 more)

### Community 14 - "profile-form.tsx"
Cohesion: 0.11
Nodes (18): Education, emptyProfile(), Experience, Profile, ProfileForm(), emptyForm, Template, TemplateList() (+10 more)

### Community 15 - "Work Plan — Pake Mail V2"
Cohesion: 0.12
Nodes (15): 1.1 Replies Module, 1.2 Bounce/Error Visibility, 2.1 Reply Rate per Template, 2.2 Dashboard Ringkasan (Enhanced), 3.1 Smart Attachment, 3.2 Peringatan Re-apply, 3.3 Notifikasi Sederhana, Catatan Implementasi (+7 more)

### Community 16 - "batch-wizard.tsx"
Cohesion: 0.18
Nodes (9): BatchWizard(), DAYS, Document, EmailAccount, Recipient, STEPS, Template, Input (+1 more)
...
Cohesion: 0.16
Nodes (7): CATEGORIES, CATEGORY_OPTIONS, Document, DocumentList(), Select, SelectProps, { handlers, signIn, signOut, auth }
...
Cohesion: 0.20
Nodes (9): 14. GLOBAL / TEMA, 15. USER FLOW, 1. LANDING PAGE (Halaman Depan — `/`), 4. DASHBOARD PAGE (`/dashboard`), Data / State, Komponen, Komponen, Pake Mail — UI Design Reference untuk Figma (+1 more)
...
Cohesion: 0.14
Nodes (12): EmailAccount, EmailAccountsList(), BatchRecipient, LABEL_OPTIONS, Pagination, Reply, ReplyBatch, ReplyList() (+4 more)
...
Cohesion: 0.67
Nodes (3): 2. HALAMAN LOGIN & REGISTER (`/login`, `/register`), Data / State, Komponen

## Knowledge Gaps
- **348 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `DashboardStats`, `geistSans`, `jetBrainsMono` (+343 more)
...
- **Why does `Dialog()` connect `cn` to `dependencies`, `document-list.tsx`, `recipient-list.tsx`, `profile-form.tsx`?**
...
- **Should `requireUserId` be split into smaller, more focused modules?**
  _Cohesion score 0.09621489621489622 - nodes in this community are weakly interconnected._
